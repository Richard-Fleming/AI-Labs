...
void Flock::myFormation(int fLeader)
{
	...
	for (int i = 0; i < fSize; i++)
	{
		...
		if (i == fLeader) {		//Deal with our leader here
			...
			targetSlot.x = targetSlot.x + radius * sin(-angleAroundCircle) * atan(angleAroundCircle);
			targetSlot.y = targetSlot.y + radius / cos(angleAroundCircle) * tan(-angleAroundCircle);
